// Copyright  (C)  2007-2008  Ruben Smits <ruben dot smits at mech dot kuleuven
// dot be> Copyright  (C)  2008  Mikael Mayer Copyright  (C)  2008  Julia Jesse
...
public:
   ...
   * @return
   */
  ChainIkSolverPos_NR_JL(const Chain& chain, const JntArray& q_min, const JntArray& q_max, ChainFkSolverPos& fksolver,
                         ChainIkSolverVel& iksolver, unsigned int maxiter = 100, double eps = 1e-6);
   ...
   * @return
   */
  ChainIkSolverPos_NR_JL(const Chain& chain, ChainFkSolverPos& fksolver, ChainIkSolverVel& iksolver,
                         unsigned int maxiter = 100, double eps = 1e-6);

  ~ChainIkSolverPos_NR_JL();

  /**
   * Calculates the joint values that correspond to the input pose given an
   * initial guess.
   ...
   * @return E_MAX_ITERATIONS_EXCEEDED if the maximum number of iterations was
   * exceeded before a result was found E_NOT_UP_TO_DATE if the internal data is
   * not up to date with the chain E_SIZE_MISMATCH if the size of the
   * input/output data does not match the chain.
   */
